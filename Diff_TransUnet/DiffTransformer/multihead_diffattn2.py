...
class MultiheadDiffAttn(nn.Module):
    def __init__(
        ...
        self.head_dim = embed_dim // num_heads // (2*8)
        self.scaling = self.head_dim ** -0.5
        
        self.q_proj = nn.Linear(embed_dim, embed_dim//8, bias=False)
        self.k_proj = nn.Linear(embed_dim, embed_dim // 8*self.n_rep, bias=False)
        self.v_proj = nn.Linear(embed_dim, embed_dim // 8*self.n_rep, bias=False)
        self.out_proj = nn.Linear(embed_dim//8, embed_dim, bias=False)
    ...
    def forward(
        ...
        q = self.q_proj(x)      #3-8-64
        k = self.k_proj(x)      #3-8-64
        v = self.v_proj(x)      #3-8-64

        q = q.view(bsz, tgt_len, 2 * self.num_heads, self.head_dim)  #3-8-4-16
        k = k.view(bsz, src_len, 2 * self.num_kv_heads, self.head_dim) #3-8-4-16
        v = v.view(bsz, src_len, self.num_kv_heads, 2 * self.head_dim) #3-8-2-32
        q = q.transpose(1, 2)  #3-4-8-16
        k = repeat_kv(k.transpose(1, 2), self.n_rep) #3-4-8-16
        v = repeat_kv(v.transpose(1, 2), self.n_rep) #3-2-8-32
        q *= self.scaling #3-4-8-16
        attn_weights = torch.matmul(q, k.transpose(-1, -2)) #3-4-8-8
        ...
        attn_weights = attn_weights[:, :, 0] - lambda_full * attn_weights[:, :, 1]  ##3-2-2-8
        
        attn = torch.matmul(attn_weights, v) #3-2-8-32
        # attn = self.attn_dropout(attn)
        attn = self.subln(attn)   #3-2-8-32
        attn = attn * (1 - self.lambda_init) #3-2-8-32
        attn = attn.transpose(1, 2).reshape(bsz, tgt_len, self.num_heads * 2 * self.head_dim) #3-8-64
        attn = self.out_proj(attn)
        return attn


class DiffTransformerLayer2(nn.Module):
    """
...
if __name__ == '__main__':
    input = torch.rand(3, 8, 512)
    model = DiffTransformerLayer2(512)
    output = model(input)
